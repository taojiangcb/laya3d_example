export class CameraMoveScript extends Laya.Script {
    ...
    protected updateCamera(elapsedTime: number): void {
        ...
        if (!isNaN(this.lastMouseX) && !isNaN(this.lastMouseY)) {
            var scene = (<any>this.owner).scene;

            if(Laya.KeyBoardManager.hasKeyDown(87)) {this.camera.moveForward(-0.002 * elapsedTime); console.log(this.camera.transform.position.valueToString());} //W
            if(Laya.KeyBoardManager.hasKeyDown(83)) {this.camera.moveForward(0.002 * elapsedTime); console.log(this.camera.transform.position.valueToString());}//S
            if(Laya.KeyBoardManager.hasKeyDown(65)) { this.camera.moveRight(-0.002 * elapsedTime);console.log(this.camera.transform.position.valueToString());}//A
            if(Laya.KeyBoardManager.hasKeyDown(68)) {this.camera.moveRight(0.002 * elapsedTime);console.log(this.camera.transform.position.valueToString());}//D
            if(Laya.KeyBoardManager.hasKeyDown(81)) {this.camera.moveVertical(0.002 * elapsedTime);console.log(this.camera.transform.position.valueToString())};//Q
            if(Laya.KeyBoardManager.hasKeyDown(69)) {this.camera.moveVertical(-0.002 * elapsedTime);console.log(this.camera.transform.position.valueToString())};//E
