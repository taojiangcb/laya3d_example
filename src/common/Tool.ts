class Tool {
    ...
    public static linearModel(sprite3D: Laya.Sprite3D, phasorSpriter3D: Laya.PhasorSpriter3D, color: Laya.Vector4, vertex1: Laya.Vector3, vertex2: Laya.Vector3, vertex3: Laya.Vector3): void {
        ...
        if (sprite3D instanceof Laya.MeshSprite3D) {
            ...
            var vbBufferData: Float32Array = mesh._vertexBuffers ? mesh._vertexBuffers[0].getData() : (<any>mesh)._vertexBuffer.getData();
            var ibBufferData: Uint16Array = mesh._indexBuffer.getData();
            var vertexStrideCount: number = mesh._vertexBuffers ? mesh._vertexBuffers[0].vertexDeclaration.vertexStride / 4 : (<any>mesh)._vertexBuffer.vertexDeclaration.vertexStride / 4;
